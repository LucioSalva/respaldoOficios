...
<div class="d-flex align-items-center justify-content-between mb-4">
    ...
    <a href="/usuarios/crear" class="btn btn-institucional btn-lg">
        ...
        <table class="table table-hover table-lg mb-0" aria-label="Lista de usuarios">
            ...
            <tr class="<?= !$u['activo'] ? 'table-secondary opacity-75' : '' ?>">
                <td class="ps-4 fw-semibold">
                    ...
                    <span class="badge bg-info ms-2 small">Tú</span>
                    <?php endif; ?>
                </td>
                <td>
                ...
                <td class="pe-4 text-center">
                    <div class="d-flex gap-2 justify-content-center">
                        <a href="/usuarios/<?= $u['id'] ?>/editar" class="btn btn-sm btn-outline-secondary">
                            <i class="fa-solid fa-pen-to-square" aria-hidden="true"></i>
                        </a>
                        <?php if ($u['id'] !== Auth::userId()): ?>
                        <form method="POST" action="/usuarios/<?= $u['id'] ?>/toggle"
                              data-confirm="<?= $u['activo'] ? '¿Desactivar a' : '¿Activar a' ?> <strong><?= htmlspecialchars($u['nombre']) ?></strong>?"
                              data-confirm-icon="question"
                              data-confirm-btn="<?= $u['activo'] ? 'Sí, desactivar' : 'Sí, activar' ?>">
