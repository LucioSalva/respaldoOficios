...
<nav class="navbar navbar-dark bg-institucional navbar-expand-lg fixed-top shadow-sm" role="navigation" aria-label="Navegación principal">
    <div class="container-fluid px-4">
        <a class="navbar-brand fw-bold d-flex align-items-center gap-2" href="<?= url_path('/dashboard') ?>">
        ...
        <div class="collapse navbar-collapse" id="navMenu">
            <?php
                // Para resaltar el item activo, comparamos REQUEST_URI sin el base path.
                $__reqUri = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH) ?? '/';
                $__bp     = base_path();
                if ($__bp !== '' && strpos($__reqUri, $__bp) === 0) {
                    $__reqUri = substr($__reqUri, strlen($__bp));
                    if ($__reqUri === '' || $__reqUri === false) { $__reqUri = '/'; }
                }
            ?>
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link nav-link-lg <?= str_starts_with($__reqUri, '/dashboard') ? 'active' : '' ?>"
                       href="<?= url_path('/dashboard') ?>">
                        <i class="fa-solid fa-gauge-high" aria-hidden="true"></i> Dashboard
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-lg <?= str_starts_with($__reqUri, '/oficios') ? 'active' : '' ?>"
                       href="<?= url_path('/oficios') ?>">
                        <i class="fa-solid fa-folder-open" aria-hidden="true"></i> Oficios
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-lg <?= str_starts_with($__reqUri, '/vacaciones') ? 'active' : '' ?>"
                       href="<?= url_path('/vacaciones') ?>">
                        <i class="fa-solid fa-umbrella-beach" aria-hidden="true"></i> Vacaciones
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-lg <?= str_starts_with($__reqUri, '/incidencias') ? 'active' : '' ?>"
                       href="<?= url_path('/incidencias') ?>">
                        <i class="fa-solid fa-triangle-exclamation" aria-hidden="true"></i> Incidencias
                    </a>
                </li>
                <?php if (Auth::hasRole([ROL_GOD, ROL_ADMIN])): ?>
                <li class="nav-item">
                    <a class="nav-link nav-link-lg <?= str_starts_with($__reqUri, '/personal') ? 'active' : '' ?>"
                       href="<?= url_path('/personal') ?>">
                        <i class="fa-solid fa-id-badge" aria-hidden="true"></i> Personal
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-lg <?= str_starts_with($__reqUri, '/usuarios') ? 'active' : '' ?>"
                       href="<?= url_path('/usuarios') ?>">
                        <i class="fa-solid fa-users" aria-hidden="true"></i> Usuarios
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-lg <?= str_starts_with($__reqUri, '/catalogos') ? 'active' : '' ?>"
                       href="<?= url_path('/catalogos') ?>">
                        <i class="fa-solid fa-table-list" aria-hidden="true"></i> Catálogos
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-lg <?= str_starts_with($__reqUri, '/importar') ? 'active' : '' ?>"
                       href="<?= url_path('/importar') ?>">
            ...
            <ul class="navbar-nav ms-auto">
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow">
                        <li>
                            <a class="dropdown-item py-2" href="<?= url_path('/logout') ?>">
...
<!-- Configuracion global del front (base path para fetch/AJAX y links dinamicos) -->
<script>window.APP_BASE_PATH = "<?= htmlspecialchars(base_path(), ENT_QUOTES, 'UTF-8') ?>";</script>
<!-- Bootstrap JS -->
