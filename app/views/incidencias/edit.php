...
<div class="d-flex align-items-center justify-content-between mb-4">
    ...
    <a href="<?= url_path('/incidencias/'.(int)$inc['id']) ?>" class="btn btn-outline-secondary btn-lg">
...
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <div class="alert alert-info">
            ...
            â€” NÂ° <?= htmlspecialchars($inc['numero_empleado'] ?? 's/n') ?>
            (<?= htmlspecialchars($inc['tipo_personal_nombre']) ?>)
            <br>
            <small class="text-muted">
                Capturado por: <?= htmlspecialchars($inc['capturo_nombre'] ?? 'â€”') ?>
                Â· Creado: <?= htmlspecialchars(date('d/m/Y H:i', strtotime($inc['created_at']))) ?>
                Â· Ãšltima ediciÃ³n: <?= htmlspecialchars(date('d/m/Y H:i', strtotime($inc['updated_at']))) ?>
            </small>
        </div>

        <form method="POST" action="<?= url_path('/incidencias/'.(int)$inc['id'].'/editar') ?>" novalidate>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <select id="quincena" name="quincena" class="form-select form-select-lg">
                        ...
                        <option value="1" <?= (string)($data['quincena'] ?? '') === '1' ? 'selected' : '' ?>>1Âª</option>
                        <option value="2" <?= (string)($data['quincena'] ?? '') === '2' ? 'selected' : '' ?>>2Âª</option>
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    <label for="folio_justificacion" class="form-label fw-semibold">Folio de justificaciÃ³n</label>
                ...
                <div class="col-12">
                    <label for="justificacion" class="form-label fw-semibold">JustificaciÃ³n formal</label>
            ...
            <div class="d-flex gap-2 mt-4">
                ...
                <a href="<?= url_path('/incidencias/'.(int)$inc['id']) ?>" class="btn btn-outline-secondary btn-lg">Cancelar</a>
            </div>
        </form>
    </div>
</div>
