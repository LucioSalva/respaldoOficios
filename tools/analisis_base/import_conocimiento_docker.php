<?php
/**
 * Importador OFICIOS DE CONOCIMIENTO - agrega sin truncate.
 * CONOCIMIENTO (tipo=3) no tiene check constraint - usamos dependencia_id.
 * CSV esperado en /tmp/CONOCIMIENTO_corregida.csv
 */
...
$csvPath = '/tmp/CONOCIMIENTO_corregida.csv';

if (!is_file($csvPath)) { fwrite(STDERR, "ERROR: no se encontro $csvPath\n"); exit(1); }

echo "========================================================\n";
echo " IMPORTADOR OFICIOS DE CONOCIMIENTO (APPEND)\n";
echo "========================================================\n";
echo " CSV  : $csvPath\n";
...
$tipoConoc = (int)$pdo->query("SELECT id FROM tipos_oficio WHERE clave='CONOCIMIENTO'")->fetchColumn();
$usuarioCapturo = (int)$pdo->query("SELECT id FROM usuarios WHERE activo=TRUE ORDER BY rol_id ASC, id ASC LIMIT 1")->fetchColumn();
if (!$tipoConoc || !$usuarioCapturo) {
    fwrite(STDERR, "ERROR: falta tipo CONOCIMIENTO o usuario activo\n"); exit(2);
}
echo "[info] usuario_capturo=$usuarioCapturo  tipo_conocimiento=$tipoConoc\n\n";
...
try {
    $sql = "INSERT INTO oficios (numero_folio,anio_folio,folio_direccion,tipo_oficio_id,dependencia_id,estado_id,asunto,observaciones,fecha_recepcion,fecha_acuse,realizo,usuario_capturo_id) VALUES (:numero_folio,:anio_folio,:folio_direccion,:tipo_oficio_id,:dependencia_id,:estado_id,:asunto,:observaciones,:fecha_recepcion,:fecha_acuse,:realizo,:usuario_capturo_id) RETURNING id";
    $st = $pdo->prepare($sql);
    ...
    foreach ($rows as $i=>$row) {
        ...
        if ($asunto===''||$fechaRec==='') { $skips[]="linea $linea: asunto/fecha vacio"; $skip++; continue; }
        $estadoNom=strtoupper(trim($row['estado_nombre']??'DE CONOCIMIENTO'));
        $estadoId=$estadosMap[$estadoNom]??$estadosMap['DE CONOCIMIENTO'];
